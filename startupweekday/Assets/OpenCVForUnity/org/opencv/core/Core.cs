...
namespace OpenCVForUnity
{
		public class Core
		{
				...
				public const int	DEPTH_MASK_8U = 1 << CV_8U;
				public const int	DEPTH_MASK_8S = 1 << CV_8S;
				public const int	DEPTH_MASK_16U = 1 << CV_16U;
				public const int	DEPTH_MASK_16S = 1 << CV_16S;
				public const int	DEPTH_MASK_32S = 1 << CV_32S;
				public const int	DEPTH_MASK_32F = 1 << CV_32F;
				public const int	DEPTH_MASK_64F = 1 << CV_64F;
				public const int	DEPTH_MASK_ALL = (DEPTH_MASK_64F << 1) - 1;
				public const int	DEPTH_MASK_ALL_BUT_8S = DEPTH_MASK_ALL & ~DEPTH_MASK_8S;
				public const int	DEPTH_MASK_FLT = DEPTH_MASK_32F + DEPTH_MASK_64F;
 ...
 * OpenCV library:</p>
 * <ul>
 *   <li> TBB 窶・The number of threads, that OpenCV will try to use for
 * parallel regions.
 * </ul>
 ...
 * by TBB library.</p>
 * <ul>
 *   <li> OpenMP 窶・An upper bound on the number of threads that could be used
 * to form a new team.
 *   <li> Concurrency 窶・The number of threads, that OpenCV will try to use for
 * parallel regions.
 *   <li> GCD 窶・Unsupported; returns the GCD thread pool limit (512) for
 * compatibility.
 *   <li> C= 窶・The number of threads, that OpenCV will try to use for parallel
 ...
 * OpenCV library:</p>
 * <ul>
 *   <li> TBB 窶・Unsupported with current 4.1 TBB release. May be will be
 * supported in future.
 *   <li> OpenMP 窶・The thread number, within the current team, of the calling
 * thread.
 *   <li> Concurrency 窶・An ID for the virtual processor that the current
 * context is executing on (0 for master thread and unique number for others,
 * but not necessary 1,2,3,...).
 *   <li> GCD 窶・System calling thread's ID. Never returns 0 inside parallel
 * region.
 *   <li> C= 窶・The index of the current parallel task.
 * </ul>
 *
 ...
 * <ul>
 *   <li> TBB 窶・User-defined parallel constructions will run with the same
 * threads number, if another does not specified. If late on user creates own
 * scheduler, OpenCV will be use it.
 *   <li> OpenMP 窶・No special defined behaviour.
 *   <li> Concurrency 窶・If <code>threads == 1</code>, OpenCV will disable
 * threading optimizations and run it's functions sequentially.
 *   <li> GCD 窶・Supports only values <= 0.
 *   <li> C= 窶・No special defined behaviour.
 * </ul>
 *
 ...
 */
		...
		// C++:  bool clipLine(Rect imgRect, Point& pt1, Point& pt2)
		[DllImport("__Internal")]
		private static extern bool core_Core_clipLine_10(int imgRect_x, int imgRect_y, int imgRect_width, int imgRect_height, double pt1_x, double pt1_y, double[] pt1_out, double pt2_x, double pt2_y, double[] pt2_out);
		...
		[DllImport("__Internal")]
		private static extern void core_Core_n_1getTextSize(string text, int fontFace, double fontScale, int thickness, int[] baseLine, double[] vals);
				...
				[DllImport("opencvforunity")]
				private static extern void core_Core_n_1getTextSize (string text, int fontFace, double fontScale, int thickness, [In, Out, MarshalAs(UnmanagedType.LPArray, SizeConst = 1)] int[] baseLine, [In, Out, MarshalAs(UnmanagedType.LPArray, SizeConst = 2)] double[] vals);
